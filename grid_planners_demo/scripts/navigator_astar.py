...
class Navigator:
    ...
    def planning_callback(self):
        ...
        #! Defining the goal state as nodes, once before the loop as it won't change
        goal = Node.from_pose(self.goal)

        #! starting with a blank map
        map = None

        #! Creating a while loop that will help with the replanning 
        
        while not rospy.is_shutdown() and not self.is_goal_reached:
            
            #! 1. Define start state as nodes, and as it is in the loop start pose will be the current pose, while goal remains the same unless changed

            start = Node.from_tf(self.robot_position, self.robot_orientation)
            

            #! 2. obtaining the updated map with each iteration 
            map = rospy.wait_for_message(self.map_topic, OccupancyGrid, timeout=5)

            if map:
                self.is_map_loaded = True

            #! 3. Redefine Collision Checker

            collision_checker = CollisionChecker(map, self.robot_radius, self.max_planning_bounds)

            #! 4. Rechecking if Start and Goal states are still valid
            ...
            if (
                ...
                rospy.logwarn("Goal can't be reached, either start or goal are in collision")
            ...
            #! 6. Solve the query the again

            rospy.loginfo("Searching solution path...")
            solved = planner.solve()  # solve the query

            if solved:  # if the query is solved
                self.stop_motion_publisher.publish(False) # Although don't need it but it made sense to me
                ...
                #! If the path is not feasible, this will stop the robot so that it can replan the path
                self.stop_motion_publisher.publish(True)
            #! Giving sometime before replanning
            rospy.sleep(1)
