...
class Navigator:
    ...
    def planning_callback(self):
        ...
        if solved:  # if the query is solved
            ...
            #! 8. Loop the search of a path in case the current path is not collision free
            while not self.is_goal_reached and not self.is_goal_cancelled:
                start = Node.from_tf(self.robot_position, self.robot_orientation)
                map = rospy.wait_for_message("/projected_map", OccupancyGrid, timeout=5)

                #! 9. Define Collision Checker again
                collision_checker = CollisionChecker(map, self.robot_radius)
                occ_nodes = []
                occ_nodes = collision_checker.is_path_free(planner.get_solution_path())

                if len(occ_nodes) > 0:
                    self.stop_motion_publisher.publish(True)

                    solved = planner.update_path(collision_checker, start, occ_nodes)
                    ...
                    if solved:
                        ...
                        rospy.logwarn("No new path found")
                rate.sleep()
